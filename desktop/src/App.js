...
const App = () => {
  ...
  const addDataPoint = () => {
    if (month && ordersInHand && monthlySales) {
      const annualSales = data.length > 0 ? data[data.length - 1].annualSales + Number(monthlySales) : Number(monthlySales);
      setData([...data, { name: month, ordersInHand: Number(ordersInHand), monthlySales: Number(monthlySales), annualSales }]);
      setMonth("");
      setOrdersInHand("");
      setMonthlySales("");
    }
  ...
  return (
    <div style={{ textAlign: "center", padding: "30px", fontFamily: "Arial, sans-serif", backgroundColor: "#1B1F23", color: "#D4D700", minHeight: "100vh" }}>
      ...
      <h2 style={{ fontSize: "30px", fontWeight: "bold", color: "#00FF9F", textShadow: "2px 2px 10px rgba(0, 255, 159, 0.7)" }}> Stabiliant Business Analytics Dashboard</h2>
      ...
      <div style={{ backgroundColor: "#222", padding: "20px", borderRadius: "10px", boxShadow: "0px 4px 10px rgba(0,255,159,0.3)", width: "60%", margin: "auto", marginBottom: "20px" }}>
        ...
        <button onClick={addDataPoint} 
          style={{ padding: "10px 15px", margin: "10px", backgroundColor: "#00FF9F", color: "#222", border: "none", borderRadius: "5px", cursor: "pointer", fontSize: "16px" }}>
          ➕ Add Data
        </button>
      </div>
      ...
      {/* Line Chart */}
      <div style={{ backgroundColor: "#222", padding: "20px", borderRadius: "10px", boxShadow: "0px 4px 10px rgba(0,255,159,0.3)", width: "80%", margin: "auto", marginTop: "20px" }}>
        <h3 style={{ color: "#00FF9F" }}>📈 Annual Sales Trend</h3>
      ...
      <button onClick={saveChartAsImage} 
        style={{ padding: "10px 20px", margin: "20px", backgroundColor: "#FF5C5C", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontSize: "16px" }}>
        📥 Save as JPG
      </button>
